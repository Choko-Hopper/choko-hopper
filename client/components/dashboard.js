...
const Dashboard = (props) => {
    ...
    if (props.user.isAdmin) {
        ...
        sidebarLinksArr = ['Order History', 'Authorized Users', 'View All Orders', 'Edit Products']
        panelArr = [orderDetails, <AllUsers key="allusers" />, <AllOrders key="allorders" />, <AllProducts key="allproducts"  path={props.path}/>]
    } else  {
        orderDetails = props.match.params.orderId ? <OrderLineItems userId={props.user.id} match={props.match} history={props.history} /> : <OrderHistory userId={props.user.id} history={props.history} />
                    ...
                    <div className="sidebar col-3">
                        ...
                        <a className="nav-link active" id="v-pills-userinfo-tab" data-toggle="pill" href="#v-pills-userinfo" role="tab" aria-controls="v-pills-userinfo" aria-selected="false">User Info</a>
                            {sidebarLinksArr.map( link => {
                                ...
                                return <a key={substr} className="nav-link" id={`v-pills-${substr}-tab`} data-toggle="pill" href={`#v-pills-${substr}`} role="tab" aria-controls={`v-pills-${substr}`} aria-selected="true">{link}</a>
                            })}
                        </div>
                    </div>
                    <div className="col-9">
                        <div className="tab-content" id="v-pills-tabContent">
                            ...
                            {panelArr.map( (panel, i) => {
                                return <div key={subStrArr[i]} className="tab-pane fade" id={`v-pills-${subStrArr[i]}`} role="tabpanel" aria-labelledby={`v-pills-${subStrArr[i]}-tab`}>{panel}</div>
                            })}
                        </div>
                    </div>
                </div>
